    ...
    $('#fileupload').fileupload({
        ...
        progressall: function(e, data) {
            ...
            if (response.success) {
				...
				var _form = $(response.form);
				if( ! groupChange) {
					groupChange = $("#group-change");
					// var fields = [];
					_form.find(".fields li").each(function(i, li) {
						...
						var field = $("<div class='field'><label>"+label+"</label><div class='field-content'><div class='input'><button class='button' style='float:right;'>Изменить везде</button><input type='"+type+"' name='"+name+"'></div></div></div>")
							.css({'margin-bottom': '1rem'});
						groupChange.append(field);

						var changeButton = field.find("button");
						var ginput = field.find("input");
						if (name == 'tags'){
							ginput.tagit(window.tagit_opts);
							field.find(".tagit").css({'width': '70%', 'margin': 0});
						}
						if (type != 'checkbox'){
							ginput.css({'width': '70%'});
						}

						// groupChange.append(changeButton);
						changeButton.on("click", function(){
							// $(fields).each(function(i, el){
								// if (input.val() == '') return;
								// var name = input.attr('name');
								if (name == 'tags') {
									$("input[id$='-"+name+"']").tagit('removeAll');
									$(ginput.val().split(",")).each(function(i, tag){
										$("input[id$='-"+name+"']").tagit('createTag', tag);
									});
								} else if (type == 'checkbox') {
									ginput.val() == 'on' ? $("input[id$='-"+name+"']").prop('checked', 'checked') : $("input[id$='-"+name+"']").removeProp('checked');
								} else {
									$("input[id$='-"+name+"']").val(ginput.val());
								}
							// });
						});
					});

					var updateButton = $("<button class='allUpdate button'>Обновить на сервере все загруженные фотографии</button>");
					groupChange.append(updateButton);
					updateButton.on("click", function(){
						$("#upload-list input[type='submit']").each(function(i, el){
							setTimeout(function(){$(el).trigger("click");}, (i+1)*300);
						});
					});
