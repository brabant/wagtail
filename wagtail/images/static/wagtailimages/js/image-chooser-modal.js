console.log("IMAGE_CHOOSER_MODAL_ONLOAD_HANDLERS");
IMAGE_CHOOSER_MODAL_ONLOAD_HANDLERS = {
    'chooser': function(modal, jsonData) {
        ...
        function ajaxifyLinks (context) {
			...
			if( $("header", modal.body).hasClass('multiple-modal') ){
				if ( ! selectMultiple) {
					selectMultiple = $('<button style="float: right;" class="button">Выбрать</button>');
					modal.body.find('h2').before(selectMultiple);
					selectMultiple.on('click', function(){
						console.log("select");
						var selected = [];
						modal.body.find('a.selected').sort(function (a, b) {
						  var indexA =parseInt( $(a).data('index'));
						  var indexB =parseInt( $(b).data('index'));
						  return (indexA < indexB) ? -1 : (indexA > indexB) ? 1 : 0;
  					    }).each(function(i, a) {
							selected.push($(a).attr('href').split('/').slice(-2)[0]);
						});
						modal.loadUrl('/admin/images/chooser/multiple/' + selected.join(',') + '/');
					});
				}
				$(this).toggleClass('selected').data('index', index++);
			} else {
